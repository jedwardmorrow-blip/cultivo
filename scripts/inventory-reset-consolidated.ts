...
/**
 * ⚠️  IMPORTANT: This script resets INVENTORY data only
 *
 * DO NOT modify or delete:
 * - orders table (contains historical sales data)
 * - order_items table (contains order line items)
 * - customers table (contains customer records)
 *
 * If historical orders are lost, restore them using:
 * scripts/import-orders.ts
 */

// CSV Data
const CSV_DATA = `package_id,product_name,category,strain,batch_number,available_qty,unit,room,status,tags
...
251021-CHL-02,Bucked Smalls - Chemlatto,Flower - Bucked,Chemlatto,250916-CHL,182.0,g,Dry Room,Available,
...
251021-DIN-01,Bulk Smalls - Dante's Inferno,Flower - Bulk,Dante's Inferno,250318HL,85.3,g,Inventory,Packaged,
...
251021-DDR-01,Bulk Flower - Devil Driver,Flower - Bulk,Devil Driver,250704H,390.3,g,Inventory,Trimmed,Machine Trimmed
251021-DDR-02,Trim - Devil Driver,Trim - Bulk,Devil Driver,250704H,684.0,g,Inventory,Trim,
...
async function main() {
  ...
  const inventoryRows = parseCSV(CSV_DATA);
  console.log(`✅ Parsed ${inventoryRows.length} inventory items\n`);

  // Extract unique batch numbers
  const uniqueBatches = [...new Set(inventoryRows.map(r => r.batch_number).filter(b => b))];
  console.log(`📦 Found ${uniqueBatches.length} unique batches:`);
  console.log(uniqueBatches.join(', '));
  console.log();

  // Step 2: Skip batch validation (batches already created via SQL)
  console.log('🔍 Step 2: Skipping batch validation (batches pre-created via SQL)...\n');

  // We'll set batch_id to null and let the system handle it
  const batchMap = new Map();

  // Step 3: Get product stages
  console.log('🎯 Step 3: Loading product stages...');
  ...
  console.log(`✅ Loaded ${stages.length} product stages\n`);

  // Step 4: Drop deprecated tables
  console.log('🗑️  Step 4: Dropping deprecated tables...');
  ...
  // Step 5: Clear active tables
  console.log('🧹 Step 5: Clearing active inventory tables...');

  await supabase.from('inventory_items').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  await supabase.from('inventory_changes').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  await supabase.from('batch_stage_tracking').delete().neq('id', '00000000-0000-0000-0000-000000000000');

  console.log('✅ Cleared inventory_items, inventory_changes, batch_stage_tracking\n');

  // Step 6: Use existing snapshot (created via SQL)
  console.log('📸 Step 6: Using existing inventory snapshot...');
  const snapshot = {id: '1fb13a64-ee11-4b09-aa6f-f0748819bec8'};
  console.log(`✅ Using snapshot: ${snapshot.id}\n`);

  // Step 7: Map stage names
  function mapProductNameToStage(productName: string, category: string): string | null {
  ...
  // Step 8: Insert inventory items
  console.log('📦 Step 7: Inserting inventory items...');

  const itemsToInsert = inventoryRows.map(row => {
    const batchId = batchMap.get(row.batch_number);
    const stageName = mapProductNameToStage(row.product_name, row.category);
  ...
  for (let i = 0; i < itemsToInsert.length; i += batchSize) {
    ...
    console.log(`✅ Inserted items ${i + 1}-${Math.min(i + batchSize, itemsToInsert.length)}`);
  }

  console.log(`\n✅ Successfully inserted ${itemsToInsert.length} inventory items\n`);

  // Step 9: Generate summary
  console.log('📊 Step 8: Generating summary...');
  ...
  inventoryRows.forEach(row => {
    const stageName = mapProductNameToStage(row.product_name, row.category) || 'Unknown';
    summary.byStage[stageName] = (summary.byStage[stageName] || 0) + 1;
    summary.byBatch[row.batch_number] = (summary.byBatch[row.batch_number] || 0) + 1;
    if (row.unit === 'g') {
      summary.totalWeight += row.available_qty;
    }
  });

  console.log('\n' + '='.repeat(60));
  console.log('📊 INVENTORY RESET SUMMARY');
  console.log('='.repeat(60));
  console.log(`Total Items Created: ${summary.totalItems}`);
  console.log(`Total Batches: ${summary.totalBatches}`);
  console.log(`Total Weight: ${summary.totalWeight.toFixed(1)}g`);
  console.log(`Snapshot ID: ${summary.snapshotId}`);
  console.log('\nItems by Stage:');
  Object.entries(summary.byStage).forEach(([stage, count]) => {
    console.log(`  ${stage}: ${count}`);
  });
  console.log('\nTop 10 Batches:');
  Object.entries(summary.byBatch)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 10)
    .forEach(([batch, count]) => {
  ...
  console.log('='.repeat(60));
  console.log('\n✅ Inventory reset completed successfully!');
}
