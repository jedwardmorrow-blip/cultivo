...
export function VendorBillEntry() {
 ...
 <div className="text-2xl font-bold text-cult-text-primary mt-2">{
 ...
 className="flex items-center gap-2 bg-cult-accent text-cult-opaque-black px-3 py-1.5 rounded-cult text-sm font-semibold hover:bg-cult-text-muted transition-colors"
 >
 ...
 className="flex items-center gap-2 bg-cult-border text-cult-text-muted px-3 py-1.5 rounded-cult text-sm hover:text-cult-text-primary transition-colors"
 >
 Add Vendor
 </button>
 <button
 ...
 ? 'bg-cult-accent text-cult-opaque-black'
 : 'bg-cult-black/50 text-cult-text-muted border border-cult-border hover:text-cult-text-primary'
 }`}
 >
 ...
 {expandedVendor === vendor.vendor_id ? <ChevronUp className="w-4 h-4 text-cult-text-muted" /> : <ChevronDown className="w-4 h-4 text-cult-text-muted" />}
 <div className="flex-1 text-left">
 <div className="text-cult-text-primary font-medium">{vendor.vendor_name}</div>
 <div className="text-xs text-cult-text-muted">{vendor.open_bill_count} open bills</div>
 </div>
 </div>
 <div className="text-right">
 <div className="text-cult-text-primary font-mono text-sm">{fmt.currency(vendor.total_outstanding)}</div>
 ...
 <div className="text-cult-text-muted text-sm">{bill.bill_number || '—'} • {fmt.date(bill.bill_date)}</div>
 <div className="text-xs text-cult-text-muted mt-0.5">{bill.is_cogs ? 'COGS' : 'OpEx'}</div>
 </div>
 <div className="text-right">
 <div className="text-cult-text-primary font-mono text-sm">{fmt.currency(bill.amount_due)}</div>
 ...
 >
 <td className="px-4 py-3 text-cult-text-primary font-medium">{bill.vendor_name}</td>
 ...
 <td className="px-4 py-3 text-cult-text-primary text-right font-mono">{fmt.currency(bill.total_amount)}</td>
 <td className="px-4 py-3 text-cult-text-primary text-right font-mono">{fmt.currency(bill.amount_due)}</td>
 <td className="px-4 py-3 text-center">
 <span className={`text-xs font-semibold ${fmt.daysSince(bill.due_date || bill.bill_date) > 30 ? 'text-cult-danger' : 'text-cult-text-muted'}`}>
 ...
 <div className="fixed inset-0 bg-black/60 z-50" onClick={() => setSelectedBill(null)}>
 <div
 className="fixed right-0 top-0 bottom-0 w-full sm:w-96 bg-cult-surface border-l border-cult-border overflow-y-auto"
 onClick={e => e.stopPropagation()}
 >
 <div className="sticky top-0 bg-cult-surface border-b border-cult-border px-6 py-4 flex items-center justify-between">
 <h2 className="text-lg font-bold text-cult-text-primary">Bill Details</h2>
 <button onClick={() => setSelectedBill(null)} className="text-cult-text-muted hover:text-cult-text-primary">
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <div className="text-cult-text-primary font-semibold mt-1">{selectedBill.vendor_name}</div>
 <div className="text-xs text-cult-text-muted mt-0.5">{fmt.category(selectedBill.vendor_category)} • {selectedBill.is_cogs ? 'COGS' : 'OpEx'}</div>
 </div>
 ...
 <div className="text-cult-text-primary font-medium text-sm mt-0.5">{item.value}</div>
 </div>
 ))}
 </div>
 ...
 <span className="text-cult-text-primary font-mono">{fmt.currency(selectedBill.subtotal)}</span>
 </div>
 <div className="flex justify-between text-sm">
 <span className="text-cult-text-muted">Tax</span>
 <span className="text-cult-text-primary font-mono">{fmt.currency(selectedBill.tax_amount)}</span>
 </div>
 <div className="border-t border-cult-border pt-2 flex justify-between text-sm">
 <span className="text-cult-text-primary font-medium">Total</span>
 <span className="text-cult-text-primary font-mono font-bold">{fmt.currency(selectedBill.total_amount)}</span>
 </div>
 ...
 <div key={i} className="flex justify-between text-cult-text-muted">
 ...
 <div>
 <div className="text-sm text-cult-text-primary font-mono">{fmt.currency(payment.amount)}</div>
 <div className="text-xs text-cult-text-muted mt-0.5">{fmt.date(payment.payment_date)}</div>
 </div>
 {payment.notes && <div className="text-xs text-cult-text-muted max-w-xs text-right">{payment.notes}</div>}
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="text-cult-text-muted text-sm mt-1 bg-cult-black/30 rounded p-2">{selectedBill.notes}</div>
 </div>
 )}
 ...
 <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setShowNewBillModal(false)}>
 <div
 className="bg-cult-surface border border-cult-border rounded-cult w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto"
 onClick={e => e.stopPropagation()}
 >
 <div className="sticky top-0 bg-cult-surface border-b border-cult-border px-6 py-4 flex items-center justify-between">
 <h2 className="text-lg font-bold text-cult-text-primary">New Vendor Bill</h2>
 <button
 onClick={() => {
 setShowNewBillModal(false);
 resetNewBillForm();
 ...
 : 'border-cult-border hover:border-cult-accent'
 }`}
 ...
 {billUploadLoading ? (
 <>
 <Loader className="w-8 h-8 text-cult-text-muted mx-auto mb-2 animate-spin" />
 <div className="text-sm text-cult-text-muted">Parsing PDF...</div>
 </>
 ) : billFile ? (
 <>
 ...
 <>
 <Upload className="w-8 h-8 text-cult-text-muted mx-auto mb-2" />
 <div className="text-sm text-cult-text-muted">Drag invoice PDF or click to upload</div>
 ...
 className="col-span-2 bg-cult-black border border-cult-border rounded-cult px-3 py-2 text-cult-text-primary text-sm focus:outline-none focus:border-cult-accent"
 />
 <select
 value={newBillForm.vendor_category}
 onChange={e => setNewBillForm(p => ({ ...p, vendor_category: e.target.value }))}
 className="bg-cult-black border border-cult-border rounded-cult px-3 py-2 text-cult-text-primary text-sm focus:outline-none focus:border-cult-accent"
 >
 ...
 className="col-span-2 bg-cult-black border border-cult-border rounded-cult px-3 py-2 text-cult-text-primary text-sm focus:outline-none focus:border-cult-accent h-16 resize-none"
 />
 </div>
 ...
 <div>
 <div className="text-sm font-medium text-cult-text-primary">COGS (Cost of Goods Sold)</div>
 <div className="text-xs text-cult-text-muted mt-0.5">Directly related to cannabis production (IRS280E compliance)</div>
 </div>
 </div>
 ...
 className="flex-1 bg-cult-border text-cult-text-muted px-4 py-2.5 rounded-cult text-sm font-semibold hover:text-cult-text-primary transition-colors"
 >
 Cancel
 </button>
 <button
 ...
 className="flex-1 bg-cult-accent text-cult-opaque-black px-4 py-2.5 rounded-cult text-sm font-semibold hover:bg-cult-text-muted transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
 >
 {billUploadLoading ? (
 <>
 ...
 <div
 className="bg-cult-surface border border-cult-border rounded-cult w-full max-w-md"
 onClick={e => e.stopPropagation()}
 >
 <div className="border-b border-cult-border px-6 py-4">
 <h2 className="text-lg font-bold text-cult-text-primary">Record Payment</h2>
 ...
 <div className="text-3xl font-bold text-cult-text-primary mt-2">{fmt.currency(selectedBill.amount_due)}</div>
 ...
 <span className="absolute left-3 top-2.5 text-cult-text-muted">$</span>
 <input
 type="number"
 step="0.01"
 min="0"
 ...
 className="w-full bg-cult-black border border-cult-border rounded-cult pl-7 pr-3 py-2.5 text-cult-text-primary text-sm focus:outline-none focus:border-cult-accent"
 />
 </div>
 ...
 className="w-full bg-cult-black border border-cult-border rounded-cult px-3 py-2.5 text-cult-text-primary text-sm focus:outline-none focus:border-cult-accent"
 />
 </div>
 ...
 className="w-full bg-cult-black border border-cult-border rounded-cult px-3 py-2 text-cult-text-primary text-sm focus:outline-none focus:border-cult-accent h-16 resize-none"
 />
 </div>
 ...
 <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowAddVendorModal(false)}>
 <div
 className="bg-cult-surface border border-cult-border rounded-cult w-full max-w-md"
 onClick={e => e.stopPropagation()}
 >
 <div className="border-b border-cult-border px-6 py-4">
 <h2 className="text-lg font-bold text-cult-text-primary">Add New Vendor</h2>
 </div>
 ...
 >
 <div>
 <h1 className="text-3xl font-bold text-cult-text-primary">Accounts Payable</h1>
 <p className="text-cult-text-muted mt-2">Manage vendor bills, track payment status, and monitor aging</p>
 </div>
