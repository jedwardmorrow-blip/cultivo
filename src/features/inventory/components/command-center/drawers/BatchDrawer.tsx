...
function StageProgressionBar({ batch }: { batch: BatchDetailRow }) {
 ...
 <div className="flex h-4 rounded-cult overflow-hidden bg-cult-surface mt-2">
 {stages
 .filter((s) => s.weight > 0)
 .map((s) => (
 <div
...
function WeightLedger({ batch }: { batch: BatchDetailRow }) {
 ...
 <div className="bg-cult-surface-subtle rounded-cult border border-cult-border-subtle divide-y divide-cult-border-faint">
 {rows.map((r) => (
...
function DrawerSkeleton() {
 ...
 <div className="h-4 rounded bg-cult-surface animate-pulse" />
 ...
 <div className="h-24 rounded-cult bg-cult-surface animate-pulse" />
 </div>
 );
}
