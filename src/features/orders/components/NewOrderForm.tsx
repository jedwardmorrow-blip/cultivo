...
function CartPanel({
 ...
 {cartItems.length > 0 && (
 <span className="px-1.5 py-0.5 text-[10px] font-bold bg-cult-accent text-cult-opaque-black rounded-full">
 {cartItems.length}
 </span>
 )}
 </div>
 ...
 onClick={onSubmit}
 disabled={!canSubmit}
 className="w-full py-3.5 bg-cult-accent text-cult-opaque-black rounded-cult font-semibold text-body hover:bg-cult-accent-hover transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
 >
 {loading ? (
 <>
...
export function NewOrderForm({ onClose, onSuccess, cloneFrom, preSelectedCustomerId, sampleMode }: NewOrderFormProps) {
 ...
 if (dataLoading) {
 return (
 <div className="fixed inset-0 bg-cult-surface/95 flex items-center justify-center z-50">
 ...
 return (
 <div className="fixed inset-0 bg-cult-surface/95 z-50 flex flex-col">
 {/* ── Header ─────────────────────────────────────────────────────────── */}
 ...
 <span className="absolute -top-1.5 -right-2.5 w-4 h-4 bg-cult-accent text-cult-opaque-black text-[9px] font-bold rounded-full flex items-center justify-center">
 {cartItems.length}
 </span>
 )}
 </div>
 ...
 {mobileTab === tab && (
 <div className="absolute top-0 left-1/4 right-1/4 h-0.5 bg-cult-accent rounded-full" />
 )}
 </button>
 ))}
 </div>
