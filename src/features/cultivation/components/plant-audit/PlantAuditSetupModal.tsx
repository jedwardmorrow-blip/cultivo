...
export function PlantAuditSetupModal({ rooms, onStart, onCancel }: PlantAuditSetupModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="glass-modal rounded-cult shadow-glass-lg max-w-xl w-full max-h-[90vh] overflow-y-auto border border-white/[0.10]">
        <div className="sticky top-0 glass-modal border-b border-white/[0.08] px-6 py-4 rounded-t-cult">
          <div className="flex items-center gap-3">
            <ClipboardList className="w-5 h-5 text-cult-accent" />
            <div>
                ...
                activeRooms.map((room) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-cult border transition text-left ${
                        selected
                          ? 'bg-cult-accent/10 border-cult-accent/40'
        ...
        <div className="flex gap-3 justify-end px-6 pb-6 border-t border-white/[0.08] pt-4">
          ...
          <button
            ...
            onClick={handleStart}
            disabled={!canStart}
            className="px-5 py-2 rounded-xl bg-cult-accent text-cult-opaque-black font-bold hover:bg-cult-accent-hover transition disabled:opacity-40 disabled:cursor-not-allowed"
          >
