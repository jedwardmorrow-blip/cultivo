...
export function PlantGroupActionsMenu({
  ...
  const dropdown = open
    ? createPortal(
        <div
          ref={menuRef}
          className="fixed z-[9999] bg-cult-surface border border-cult-border min-w-44"
          style={{
        ...
        >
          {canAdvance && nextStage && (
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-text-primary hover:bg-cult-black transition-colors text-left"
            >
              <ArrowRight className="w-3.5 h-3.5 text-cult-border flex-shrink-0" />
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-text-primary hover:bg-cult-black transition-colors text-left"
          >
            <Home className="w-3.5 h-3.5 text-cult-border flex-shrink-0" />
            Move to Room
          </button>
          ...
          {canBeMother ? (
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-text-primary hover:bg-cult-black transition-colors text-left"
            >
              <Star className={`w-3.5 h-3.5 flex-shrink-0 ${group.is_mother ? 'text-cult-warning' : 'text-cult-border'}`} />
              {group.is_mother ? 'Remove Mother Status' : 'Mark as Mother'}
            </button>
          ) : (
            <div
              className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-surface cursor-not-allowed"
              title="Must be in Veg or Flower stage to designate as mother"
            >
              <Star className="w-3.5 h-3.5 flex-shrink-0 text-cult-surface" />
              Mark as Mother
            </div>
          )}

          <div className="border-t border-cult-surface my-0.5" />

          <button
            onClick={() => handle(onViewPlants)}
            className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-text-primary hover:bg-cult-black transition-colors text-left"
          >
            <Leaf className="w-3.5 h-3.5 text-cult-border flex-shrink-0" />
            View Plant IDs
          </button>
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-text-primary hover:bg-cult-black transition-colors text-left"
          >
            <Printer className="w-3.5 h-3.5 text-cult-border flex-shrink-0" />
            Print Group Label
          </button>
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-cult-text-primary hover:bg-cult-black transition-colors text-left"
          >
            <Info className="w-3.5 h-3.5 text-cult-border flex-shrink-0" />
            View History
          </button>
        </div>,
        document.body
  ...
  return (
    <div className="relative flex-shrink-0">
      <button
        ...
        className={`transition-colors text-cult-border hover:text-cult-text-primary ${compact ? 'p-0.5' : 'p-1.5'}`}
        title="Actions"
      >
