...
export function TaskTypesOverlay({ onClose, inline = false }: TaskTypesOverlayProps) {
 ...
 const panel = (
 <div className={`${inline ? 'w-full' : 'relative w-full max-w-4xl'} bg-cult-surface border border-cult-surface rounded-lg flex flex-col ${inline ? '' : 'max-h-[85vh]'}`}>
 {/* Header */}
 <div className="flex items-center justify-between px-5 py-4 border-b border-cult-surface/50 flex-shrink-0">
 ...
 {!inline && (
 <button type="button" onClick={onClose} className="p-2 hover:bg-cult-surface-raised rounded transition-colors">
 ...
 {loading ? (
 <div className="text-sm text-cult-border py-8 text-center">Loading task types...</div>
 ) : (
 ...
 className={`text-left bg-cult-surface border p-4 space-y-3 transition-all hover:border-cult-accent/50 ${
 !tt.is_enabled ? 'opacity-40 border-cult-surface/50' : editingId === tt.id ? 'border-cult-accent ring-1 ring-cult-accent/30' : 'border-cult-surface'
 }`}
 >
 <div className="flex items-center gap-2">
 <div
 ...
 <span className="text-xs font-semibold text-cult-text-primary uppercase tracking-wider block truncate">{tt.label}</span>
 {!tt.is_enabled && <span className="text-[10px] text-cult-warning uppercase">Disabled</span>}
 </div>
 {tt.is_builtin
 ? <Lock className="w-3 h-3 text-cult-surface flex-shrink-0" title="Built-in type — cannot be deleted" />
 : <Pencil className="w-3 h-3 text-cult-surface flex-shrink-0" />
 }
 </div>
 <p className="text-xs text-cult-text-muted leading-relaxed line-clamp-2">
 {tt.description}
 </p>
 {tt.fields.length > 0 && (
 <div className="pt-2 border-t border-cult-surface/50">
 <span className="text-xs text-cult-border uppercase tracking-wider">Fields</span>
 <div className="flex flex-wrap gap-1 mt-1">
 {tt.fields.map((f) => (
 <span key={f} className="px-1.5 py-0.5 text-xs bg-cult-surface-raised text-cult-text-muted rounded-sm">{f}</span>
 ))}
 </div>
 </div>
 )}
...
function TaskTypeEditorModal({ taskType, onSave, onDelete, onClose }: TaskTypeEditorModalProps) {
 ...
 {/* Card */}
 <div className="relative w-full max-w-lg bg-cult-surface border border-cult-surface rounded-lg overflow-hidden">
 {/* Header */}
 ...
 className="w-full px-3 py-2 bg-cult-surface-raised border border-cult-surface text-cult-text-primary text-sm rounded-sm focus:border-cult-accent focus:outline-none"
 ...
 {/* Color */}
 <div>
 <label className="block text-xs text-cult-border uppercase tracking-wider mb-1">Color</label>
 ...
 type="button"
 onClick={() => setColor(c)}
 className={`w-6 h-6 rounded-sm border-2 transition-all ${color === c ? 'border-cult-accent scale-110' : 'border-transparent'}`}
 ...
 {/* Description */}
 <div>
 <label className="block text-xs text-cult-border uppercase tracking-wider mb-1">Description</label>
 <textarea
 value={description}
 onChange={(e) => setDescription(e.target.value)}
 rows={2}
 className="w-full px-3 py-2 bg-cult-surface-raised border border-cult-surface text-cult-text-primary text-sm rounded-sm focus:border-cult-accent focus:outline-none resize-none"
 ...
 {/* Icon */}
 <div>
 <label className="block text-xs text-cult-border uppercase tracking-wider mb-1">Icon</label>
 ...
 icon === iconName ? 'border-cult-accent bg-cult-surface-raised' : 'border-transparent hover:bg-cult-surface-raised/50'
 }`}
 title={iconName}
 >
 ...
 {/* Fields */}
 <div>
 <label className="block text-xs text-cult-border uppercase tracking-wider mb-1">
 Completion Fields <span className="text-cult-surface">(comma-separated)</span>
 </label>
 <input
 type="text"
 value={fieldsStr}
 ...
 />
 <span className="text-xs text-cult-text-muted">Enabled</span>
 </label>
 )}
 </div>

 {/* Footer */}
 <div className="flex items-center justify-between px-5 py-4 border-t border-cult-surface/50 bg-cult-surface">
 <div>
 ...
 <button type="button" onClick={() => setConfirmDelete(false)} className="px-2 py-1 text-xs text-cult-border hover:text-cult-text-primary transition-colors">
 No
 </button>
 </div>
 )}
 ...
 type="button"
 onClick={onClose}
 className="px-3 py-2 text-xs text-cult-border hover:text-cult-text-primary transition-colors"
 >
 Cancel
 </button>
 <button
