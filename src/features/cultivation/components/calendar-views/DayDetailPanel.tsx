...
export function DayDetailPanel({ date, breakdown, onClose, onEditRoom }: DayDetailPanelProps) {
  ...
  return (
    <div className="bg-cult-surface-raised/40 border-b border-cult-surface/50 px-3 sm:px-5 py-3 animate-fade-in">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-cult-text-primary">{dateLabel}</span>
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-2 -m-1 hover:bg-cult-surface-raised rounded-lg transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
        >
      ...
      {breakdown.length === 0 ? (
        <p className="text-xs text-cult-border italic py-2">No scheduled tasks for this day.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {breakdown.map(({ room, taskTypes, schedules: daySchedules }) => (
            <div
              key={room.id}
              className="bg-cult-surface border border-cult-surface/50 border-l-2 p-2.5 group/card"
              style={{ borderLeftColor: ROOM_TYPE_HEX[room.room_type] ?? '#6B7280' }}
            >
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs font-bold text-cult-text-primary">{room.room_code}</span>
                ...
                {onEditRoom && (
                  <button
                    ...
                    className="p-1.5 opacity-0 group-hover/card:opacity-100 hover:bg-cult-surface-raised rounded transition-all"
                    title="Edit schedule"
                  >
                    <Pencil className="w-3 h-3 text-cult-border hover:text-cult-text-primary" />
                  </button>
                )}
              </div>
