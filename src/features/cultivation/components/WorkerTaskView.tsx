...
const GLASS_CARD = 'rounded-cult border border-cult-border bg-cult-surface';
...
function WorkerSelector({ staff, onSelect, attendance, taskCounts, onToggleAttendance }: {
 ...
 className={`${GLASS_CARD} relative flex flex-col items-center justify-center gap-2 p-5 active:scale-[0.97] transition-all duration-200 min-h-[130px] hover:bg-cult-surface-overlay hover:border-cult-border-strong ${!isPresent ? 'opacity-40' : ''} ${hasTasksButOff ? 'border-amber-500/20' : ''}`}
 >
...
function TaskRow({ task, roomName, workerId, taskAssignments, leadAssignment, feedPreview, onToggleActivity, onTap, onStart, onComplete }: {
 ...
 return (
 <div className={`border-b border-cult-border-faint ${isDone ? 'opacity-30' : ''}`}>
 {/* Main row */}
 <button
 onClick={onTap}
 className="w-full flex items-center gap-3 px-5 py-4 text-left active:bg-cult-surface-inset transition-colors min-h-[64px]"
 >
 ...
 className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider rounded-cult bg-sky-500/15 text-sky-300 border border-sky-500/20 active:scale-95 transition-all duration-200 min-h-[44px]"
 >
 ...
 className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider rounded-cult bg-emerald-500/15 text-emerald-300 border border-emerald-500/20 active:scale-95 transition-all duration-200 min-h-[44px]"
 >
 ...
 className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider rounded-cult active:scale-95 transition-all duration-200 min-h-[44px] ${
 ...
 : 'bg-cult-surface-inset text-cult-text-muted border border-cult-border hover:bg-cult-surface-raised'
 }`}
 >
 {myAssignment.is_active ? (
 <>
...
function RoomGroup({
 ...
 className="w-full flex items-center justify-between px-5 py-3 active:bg-cult-surface-inset transition-colors min-h-[48px]"
...
function QuickObservationSheet({ roomCodes, workerId, onClose }: {
 ...
 className="relative rounded-t-2xl border-t border-cult-border bg-cult-surface p-5 space-y-4 pb-8"
 >
 <div className="flex items-center justify-between">
 ...
 rows={3}
 className="w-full glass-input rounded-cult px-4 py-3 text-sm text-white placeholder:text-white/25 resize-none"
 />
 ...
 className="w-full flex items-center justify-center gap-2 rounded-cult py-3 bg-emerald-500/15 text-emerald-300 border border-emerald-500/20 text-sm font-medium active:scale-[0.98] transition-all duration-200 disabled:opacity-30"
 >
...
export function WorkerTaskView() {
 ...
 <div className={`min-h-screen ${PAGE_BG} flex flex-col`}>
 {/* Sticky header */}
 <div className="bg-cult-surface border-b border-cult-border-subtle sticky top-0 z-10">
 <ProgressHeader
 ...
 key={f}
 onClick={() => setFilter(f)}
 className={`px-3.5 py-2 text-xs font-semibold uppercase tracking-wider rounded-cult transition-all duration-200 min-h-[40px] active:scale-95 ${
 filter === f
 ? 'bg-white/10 text-white/70 border border-white/15'
 : 'text-white/30 border border-transparent hover:bg-white/5'
 }`}
 ...
 style={{ borderColor: 'rgba(16,185,129,0.2)', boxShadow: '0 024px rgba(16,185,129,0.1)' }}
 >
