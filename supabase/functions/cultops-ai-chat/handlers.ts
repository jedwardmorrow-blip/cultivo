...
export function buildSystemPrompt(
  ...
  if (persona && !u.isCreator) {
    ...
    if (persona.data_domains?.length > 0) {
      prompt += `- RELEVANT DOMAINS: ${persona.data_domains.join(", ")}\n`;
      prompt += `- CONTEXT FILTER: Prioritize relevant domains. Do NOT surface system architecture, financial strategy, or cross-departmental context UNLESS asked.\n`;
    }
...
export async function logConversation(userId: string, sessionId: string | null, userMessage: string, assistantMessage: string, messageType: string = "chat"): Promise<string> {
  const prodClient = getProductionClient();
  ...
  if (chatSessionId) { await prodClient.from("ai_chat_messages").insert([{ session_id:chatSessionId, role:"user", content:userMessage, message_type:messageType },{ session_id:chatSessionId, role:"assistant", content:assistantMessage, message_type:messageType }]); }
  return chatSessionId || "";
}

export function sseResponse(text: string, intents: string[], sessionIdPromise: Promise<string>): Response {
  const encoder = new TextEncoder();
  const stream = new ReadableStream({
    async start(controller) {
      controller.enqueue(encoder.encode(`data: ${JSON.stringify({type:"metadata",intents,financial:false})}\n\n`));
      controller.enqueue(encoder.encode(`data: ${JSON.stringify({type:"text",text})}\n\n`));
      try { const sid = await sessionIdPromise; controller.enqueue(encoder.encode(`data: ${JSON.stringify({type:"done",session_id:sid})}\n\n`)); } catch { }
      controller.close();
    },
  });
