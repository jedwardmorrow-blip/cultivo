/*
  # Fix GROUP BY Error in finalize_session_aggregated()

  1. Problem
    - Query selects strain_id without GROUP BY or aggregate function
    - Causes error: "column must appear in the GROUP BY clause or be used in an aggregate function"
    - Occurs when manager attempts to finalize packaging sessions

  2. Solution
    - Wrap strain_id in MAX() aggregate function
    - All sessions in array have same strain_id (grouped by batch+product)
    - MAX(strain_id) returns the strain_id value correctly

  3. Changes
    - Update query in packaging session branch of finalize_session_aggregated()
    - Change: strain_id → MAX(strain_id)
*/
  ...
  CASE v_session_type
    ...
    WHEN 'packaging' THEN
      ...
      IF v_session_ids IS NOT NULL AND array_length(v_session_ids, 1) > 0 THEN
        ...
        -- FIX: Wrap strain_id in MAX() to avoid GROUP BY error
        SELECT 
          MAX(strain_id),
...
COMMENT ON FUNCTION finalize_session_aggregated IS 'Finalizes completed production sessions and creates consolidated inventory items for packaging sessions. Fixed GROUP BY error in v2.';
