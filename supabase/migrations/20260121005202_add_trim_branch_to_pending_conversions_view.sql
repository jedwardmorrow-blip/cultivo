/*
  # Add Trim Branch to Pending Conversions View

  ## Problem
  pending_conversion_sessions view has 5 branches (big buds, small buds, packaging,
  bucked flower, bucked smalls) but NO branch for trim outputs. This makes trim
  invisible in the conversions workflow.

  ## Solution
  Add 6th branch to unpivot trim outputs from trim_sessions, following the same
  pattern as flower and smalls branches.

  ## Changes
  1. Drop and recreate pending_conversion_sessions view
  2. Add new UNION ALL branch for trim outputs
  3. Branch queries trim_sessions WHERE trim_grams > 0
  4. Uses output_product_trim_name for product matching
  5. Calculates remaining weight after subtracting packaged amounts

  ## Impact
  - Trim will now appear in conversions screen
  - Managers can finalize trim into inventory
...
-- =====================================================
-- Recreate view with trim branch
-- =====================================================
...
UPDATED 2026-01-21: Added Branch 3 for trim byproduct tracking.
