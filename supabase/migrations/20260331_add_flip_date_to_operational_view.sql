/*
  # Add flip_date to v_room_operational_state

  ## Problem
  The overview card (CultivationDashboard) shows "Day N" from stage_entered_at
  while the detail drawer (RoomDetailDrawer) shows "Day N" from the room section's
  flip_date. These diverge when plants enter a flower room before or after the
  official flip date is recorded.

  ## Solution
  Add a room_flip CTE that joins room_tables → room_sections to pull the earliest
  flip_date and projected_harvest_date per room. Expose:
    - earliest_flip_date        → the room's operational flip date
    - days_since_flip           → (current_date - earliest_flip_date) + 1
    - section_projected_harvest → earliest projected_harvest_date from sections
    - section_days_to_harvest   → section_projected_harvest - current_date

  The card component will prefer days_since_flip for flower rooms, falling back to
  days_in_stage for veg/clone rooms. This ensures the card and drawer always agree.

  ## Affected Objects
  - v_room_operational_state (CREATE OR REPLACE VIEW — additive columns only)
*/
...
room_flip AS (
  SELECT
    ...
    min(rs.projected_harvest_date) AS earliest_projected_harvest
  FROM room_tables rt
...
SELECT
  ...
  -- New: flip-date-based fields from room_sections
...
COMMENT ON VIEW public.v_room_operational_state IS
  'Layer 1 of the Unified Command Center. Aggregates plant groups, tasks, harvest data, flip dates, and urgency signals into a single room-level operational view. Updated 2026-03-31: added flip_date from room_sections for accurate flower room day counting.';
