...
--   - Example: Animal Tsunami 14g Smalls had 16g smalls but said 'ready' because
--     flower(1624) + smalls(16) + trim(5836) = 7476 > 2660g demand
--
-- FIX: stock_status now checks the correct inventory category per format:
--   - Flower formats → flower_bulk only
--   - Smalls formats → smalls_bulk only
--   - Trim formats → trim_bulk only
--   - Fresh Frozen → flower_bulk (bulk grams)
...
SELECT
  ...
  -- v5: Category-aware stock status
  -- Flower formats check flower_bulk, Smalls check smalls_bulk, Trim checks trim_bulk
  -- Pipeline (bucked/binned) is pre-sorting material → feeds 'needs_processing' for all categories
  CASE
    WHEN fi.product_category = 'Flower' THEN
      CASE
        WHEN COALESCE(pip.bulk_flower_g, 0) >= SUM(fi.remaining_qty * fi.weight_per_unit_g) THEN 'ready'
        WHEN COALESCE(pip.bulk_flower_g, 0) + COALESCE(pip.bucked_g, 0) + COALESCE(pip.binned_g, 0) > 0 THEN 'needs_processing'
        ELSE 'no_stock'
      END
    WHEN fi.product_category = 'Smalls' THEN
      CASE
        WHEN COALESCE(pip.bulk_smalls_g, 0) >= SUM(fi.remaining_qty * fi.weight_per_unit_g) THEN 'ready'
        WHEN COALESCE(pip.bulk_smalls_g, 0) + COALESCE(pip.bucked_g, 0) + COALESCE(pip.binned_g, 0) > 0 THEN 'needs_processing'
        ELSE 'no_stock'
      END
    WHEN fi.product_category = 'Trim' THEN
      CASE
        WHEN COALESCE(pip.trim_g, 0) >= SUM(fi.remaining_qty * fi.weight_per_unit_g) THEN 'ready'
        WHEN COALESCE(pip.trim_g, 0) + COALESCE(pip.bucked_g, 0) + COALESCE(pip.binned_g, 0) > 0 THEN 'needs_processing'
        ELSE 'no_stock'
      END
    WHEN fi.product_category = 'Fresh Frozen' THEN
      CASE
        WHEN COALESCE(pip.bulk_flower_g, 0) >= SUM(fi.remaining_qty * fi.weight_per_unit_g) THEN 'ready'
        WHEN COALESCE(pip.bulk_flower_g, 0) + COALESCE(pip.bucked_g, 0) + COALESCE(pip.binned_g, 0) > 0 THEN 'needs_processing'
        ELSE 'no_stock'
