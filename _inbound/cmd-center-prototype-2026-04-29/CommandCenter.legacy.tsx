...
const GLASS_ELEVATED = 'rounded-cult border border-cult-border-strong bg-cult-surface-raised';
const GLASS_TILE = 'rounded-cult border border-cult-border-subtle bg-cult-surface';
const GLASS_HOVER = 'hover:bg-cult-surface-raised hover:border-cult-border-strong transition-colors';
const GLASS_EMPTY = 'rounded-cult border border-cult-border-faint bg-cult-surface-inset';

// Page background — instrument-grade canvas (no gradient)
const PAGE_BG = 'bg-cult-surface-inset';
...
function statusRingStyle(urgency: number, isEmpty: boolean, roomType?: string): React.CSSProperties {
 ...
 : `0 0 8px ${color}40, 0 020px ${color}15`,
 };
}
...
function LaborOverviewPanel({ tasks, opsRooms, staff, onRoomClick, onClose }: LaborOverviewProps) {
 ...
 // Compute the7 dates for the current week (Mon-Sun)
 ...
 <div className="w-full h-2 bg-cult-surface-raised rounded-full overflow-hidden flex">
 <div
 ...
 <div className="bg-cult-surface-inset border border-cult-border-subtle rounded-cult overflow-hidden">
 ...
 className={`w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-cult-surface-subtle transition-colors active:scale-[0.99] ${
 i < roomBreakdown.length - 1 ? 'border-b border-cult-border-faint' : ''
 }`}
 >
 ...
 onClick={() => { onRoomClick(room.code); onClose(); }}
 className={`w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-cult-surface-subtle transition-colors active:scale-[0.99] ${
 i < projectedRoomBreakdown.length - 1 ? 'border-b border-cult-border-faint' : ''
 }`}
 >
 <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: stage.base }} />
 <span className="font-mono text-xs text-white/70 w-16">{room.code}</span>
 ...
 <div className="px-4 py-1.5 border-t border-cult-border-faint">
...
function ScheduleCardCompact({ state, tasks, schedules, flipDate }: {
 ...
 {/* Phase progress bar */}
 <div className="w-full h-1 rounded-full bg-cult-surface-raised mb-3">
 <div
 ...
 {/* Milestones */}
 {milestones.length > 0 && (
 <div className="space-y-1 pt-1 border-t border-cult-border-faint">
 {milestones.map(m => {
...
function DraggableTaskPill({ task, isDragOverlay }: { task: DailyTaskInstance; isDragOverlay?: boolean }) {
 ...
 className={`flex items-center gap-1.5 rounded-cult px-2.5 py-2 text-[10px] font-medium cursor-grab active:cursor-grabbing transition-opacity ${
...
function DroppableDayColumn({ dateStr, phaseDay, isToday, stageColor, children }: {
 ...
 <div
 ref={setNodeRef}
 className={`flex flex-col rounded-cult border transition-colors duration-200 ${
 isWeekend ? 'min-w-[40px] max-w-[50px]' : 'flex-1 min-w-0'
 } ${isOver ? 'bg-cult-surface-raised border-cult-border-active' : 'bg-cult-surface-inset border-cult-border-faint'} ${
 isToday ? 'border-l-2' : ''
 }`}
 style={isToday ? { borderLeftColor: `${stageColor}60` } : undefined}
 >
 {/* Day header */}
 <div className={`px-2 py-1.5 border-b border-cult-border-faint text-center ${isToday ? 'bg-cult-surface-inset' : ''}`}>
...
function ScheduleCalendarExpanded({ state, tasks, onReschedule }: {
 ...
 if (schedulesLoading) {
 return <div className="glass-skeleton h-64 rounded-cult" />;
 }
 ...
 {/* Milestones strip */}
 {milestones.length > 0 && (
 <div className="pt-3 border-t border-cult-border-subtle space-y-1.5">
...
function RoomTile({ state, tasks, isFeatured, onClick }: {
 state: RoomOperationalState;
 tasks: DailyTaskInstance[];
 isFeatured?: boolean;
 onClick: () => void;
 ...
 const inProgressTasks = roomTasks.filter(t => t.status === 'in_progress').length;
 ...
 background: `radial-gradient(circle, ${glowColor}${isLarge ? '22' : '15'} 0%, ${glowColor}06 45%, transparent70%)`,
 filter: 'blur(12px)',
 }}
 />
 );
 ...
 {/* Featured tile extras: task progress bar + upcoming tasks */}
 {isLarge && !isEmpty && roomTasks.length > 0 && (
 <div className="mt-3 space-y-2">
 {/* Segmented progress bar */}
 <div className="flex gap-0.5 h-1.5 rounded-full overflow-hidden">
 {doneTasks > 0 && (
 <div className="bg-emerald-500/60 rounded-full" style={{ flex: doneTasks }} />
 )}
 {inProgressTasks > 0 && (
 <div className="bg-amber-500/60 rounded-full" style={{ flex: inProgressTasks }} />
 )}
 {pendingTasks > 0 && (
 <div className="bg-cult-surface-overlay rounded-full" style={{ flex: pendingTasks }} />
 )}
 </div>
 {/* Upcoming task labels */}
 <div className="flex items-center gap-1.5 flex-wrap">
 {roomTasks
 .filter(t => t.status !== 'completed' && t.status !== 'skipped')
 .slice(0, 3)
 .map(t => {
 const cfg = getTaskTypeConfig(t.task_type);
 return (
 <span
 key={t.id}
 className="text-[9px] px-1.5 py-0.5 rounded-md"
 style={{ backgroundColor: `${cfg.color}12`, color: `${cfg.color}90` }}
 >
 {cfg.label}
 </span>
 );
...
function ExpandedRoomView({ state, tasks, groups, rooms, onUpdateTaskStatus, onCompleteWithLog, onAssignWorker, onCreateTask, onMoveToRoom, onSplitAndMove, onSplitAndMoveMultiple, onAdvanceStage, onBack }: {
 ...
 <button type="button" onClick={onBack} className="p-2 rounded-cult hover:bg-white/10 transition-colors active:scale-95">
 ...
 className="flex items-center gap-2 px-3.5 py-2 rounded-cult bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-medium hover:bg-amber-500/20 transition-all active:scale-95"
 >
 ...
 <div key={label} className="p-3 rounded-cult bg-cult-surface-inset border border-cult-border-faint">
 ...
 <div className="absolute inset-0 bg-black/60" onClick={() => setShowHarvestConfirm(false)} />
 <motion.div
 ...
 >
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-cult bg-amber-500/10 flex items-center justify-center">
 ...
 className="flex-1 py-2.5 rounded-cult bg-amber-500/20 text-amber-300 text-sm font-medium hover:bg-amber-500/30 transition-colors active:scale-[0.98]"
 >
 Start Harvest
 </button>
 <button
 ...
 className="px-4 py-2.5 rounded-cult border border-white/10 text-white/40 text-sm hover:border-white/20 hover:text-white/60 transition-colors"
 >
 Cancel
 </button>
 </div>
 ...
 {advancingGroups && (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
 <div className="absolute inset-0 bg-black/60" onClick={() => setAdvancingGroups(null)} />
 <motion.div
 initial={{ opacity: 0, scale: 0.95 }}
 animate={{ opacity: 1, scale: 1 }}
 exit={{ opacity: 0, scale: 0.95 }}
 ...
 const next = NEXT_STAGE[g.growth_stage];
 return (
 <div key={g.id} className="flex items-center justify-between text-[11px] px-2 py-1 rounded-lg bg-cult-surface-inset">
 ...
 className="flex-1 py-2.5 rounded-cult bg-white/5 text-white/40 text-xs active:scale-95 transition-all"
 >
 Cancel
 </button>
 <button
 ...
 className="flex-1 py-2.5 rounded-cult bg-emerald-500/20 text-emerald-300 text-xs active:scale-95 transition-all disabled:opacity-40 disabled:pointer-events-none"
 >
...
function InlineAddTask({ roomId, onAdd, onCancel }: {
 ...
 disabled={saving}
 className="flex items-center gap-1.5 py-1.5 px-3 rounded-full bg-cult-surface-inset border border-cult-border-subtle hover:bg-cult-surface-overlay hover:border-cult-border-active transition-all active:scale-95 disabled:opacity-30 whitespace-nowrap flex-shrink-0"
 >
...
function TaskChecklist({ tasks, assignments, staff, onUpdateStatus, onOpenCompletion, onAssignToTask, onPromoteToLead, getForTask }: {
 ...
 className="w-full flex items-center gap-3 py-3 px-3 rounded-cult hover:bg-white/5 transition-all duration-200 text-left group"
 >
 {/* Checkbox */}
 <button
 type="button"
...
function TaskExpandedDetail({ task, taskAssignments, staff, onUpdateStatus, onOpenCompletion, onAssignToTask, onPromoteToLead }: {
 ...
 return (
 <div className="ml-11 mb-2 p-4 rounded-cult bg-white/5 border border-white/5 space-y-3">
 <div className="flex items-center justify-between">
 ...
 className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-cult text-xs font-medium transition-all duration-200 active:scale-95 ${
 isLead
 ? 'bg-amber-500/20 border border-amber-400/30 text-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.2)]'
 : isAssigned
 ? 'bg-white/10 border border-white/20 text-white/70'
 : 'bg-cult-surface-inset border border-cult-border-subtle text-white/30 hover:bg-cult-surface-raised hover:text-white/50'
 }`}
...
function InlineTankMixRecipe() {
 ...
 <div key={i} className="flex justify-between text-[11px] py-1 px-2 rounded-lg bg-cult-surface-inset">
...
function CellHoverPopover({ strainCounts, totalPlants, tableNum, sectionLabel }: {
 ...
 <div className="rounded-cult border border-cult-border-active bg-black/80 shadow-[0_8px_32px_rgba(0,0,0,0.6)] p-3 min-w-[140px]">
 ...
 <div className="absolute left-1/2 -translate-x-1/2 -bottom-1 w-2 h-2 rotate-45 bg-black/80 border-r border-b border-cult-border-active" />
 </div>
 );
}
...
function PinnedCellPopover({ sectionId, strainCounts, totalPlants, tableNum, sectionLabel, groups, onMove, onKill, onPrint, onPrintPlants, onAdvance, onClose }: {
 ...
 className="rounded-cult border border-cult-border-active bg-black/85 shadow-[0_12px_48px_rgba(0,0,0,0.7)] p-4 min-w-[200px] max-w-[280px]"
 >
 {/* Header */}
 ...
 className="w-full flex items-center justify-between text-[11px] py-1.5 px-2 rounded-lg hover:bg-cult-surface-subtle transition-colors active:scale-[0.98]"
 >
 <div className="flex items-center gap-1.5">
 {isExpanded
 ...
 {/* Action bar */}
 {groups.length > 0 && (
 <div className="flex items-center gap-1.5 pt-3 border-t border-cult-border-subtle">
 <button
 type="button"
 onClick={() => onMove(groups)}
 className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-cult bg-cult-surface-subtle text-white/50 text-[10px] font-medium hover:bg-cult-surface-overlay transition-all active:scale-95"
 >
 ...
 className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-cult bg-red-500/10 text-red-400 text-[10px] font-medium hover:bg-red-500/20 transition-all active:scale-95"
 >
 ...
 className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-cult bg-emerald-500/10 text-emerald-400 text-[10px] font-medium hover:bg-emerald-500/20 transition-all active:scale-95"
 >
 ...
 {/* Arrow */}
 <div className="absolute left-1/2 -translate-x-1/2 -bottom-1 w-2 h-2 rotate-45 bg-black/85 border-r border-b border-cult-border-active" />
 </div>
 );
}
...
function RoomGrid({ roomId, compact, inline, expanded, groups, onMoveGroups, onKillGroups, onPrintGroups, onPrintPlants, onAdvanceGroups }: {
 ...
 if (loading) return compact ? <div className="h-12 rounded-lg bg-cult-surface-inset animate-pulse" /> : <div className={`${inline ? '' : GLASS + ' p-4'} h-32 animate-pulse`} />;
 ...
 : 'bg-cult-surface-inset border border-cult-border-faint hover:bg-cult-surface-subtle'
...
function InteractiveFeedCard({ state }: { state: RoomOperationalState }) {
 ...
 if (loading) return <div className="h-16 rounded-lg bg-cult-surface-inset animate-pulse" />;
 if (!recipe) return <p className="text-[10px] text-white/20">No feed program configured.</p>;
 ...
 <div className="flex items-center gap-4 mb-4 p-3 rounded-cult bg-cult-surface-inset border border-cult-border-subtle">
 <div className="flex-1">
 ...
 return (
 <div key={entry.product.id} className="flex justify-between items-center py-2 px-3 rounded-cult bg-cult-surface-inset border border-cult-border-faint">
 ...
 {/* pH — large editable */}
 <div className="flex items-center gap-4 p-3 rounded-cult bg-cult-surface-inset border border-cult-border-subtle mb-4">
 ...
 className={`flex-1 py-2 rounded-cult text-xs font-medium transition-all active:scale-[0.98] ${
 isDirty
 ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/25 hover:bg-emerald-500/30'
 : 'bg-cult-surface-inset text-white/20 border border-cult-border-faint cursor-default'
 }`}
 >
 ...
 type="button"
 onClick={handleReset}
 className="px-3 py-2 rounded-cult text-[10px] text-white/30 border border-cult-border-subtle hover:text-white/50 hover:border-cult-border transition-all active:scale-95"
 >
 Reset
 </button>
 )}
...
function FeedCardContent({ state }: { state: RoomOperationalState }) {
 ...
 return (
 <div key={entry.product.id} className="flex justify-between items-center py-1.5 px-2.5 rounded-lg bg-cult-surface-inset">
 <div className="flex items-center gap-2">
...
function QuickAddTask({ roomId, roomCode, staff, onAdd, onClose }: {
 ...
 className="flex items-center gap-2 p-3 rounded-cult bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all active:scale-95 text-left"
 >
 ...
 disabled={saving}
 className="w-full p-3 rounded-cult bg-white/5 border border-white/5 hover:bg-white/10 transition-all active:scale-95 text-left"
 >
 ...
 disabled={saving}
 className="w-full flex items-center gap-3 p-3 rounded-cult bg-white/5 border border-white/5 hover:bg-white/10 transition-all active:scale-95 text-left"
 >
...
export function CommandCenter() {
 ...
 // Section rooms by type, sorted by room code within each section
 const { flowerRooms, vegRooms, momRooms, defaultFeaturedId } = useMemo(() => {
 const flower = opsRooms
 .filter(r => r.room_type === 'flower')
 .sort((a, b) => a.room_code.localeCompare(b.room_code));
 const veg = opsRooms
 .filter(r => r.room_type === 'veg' || r.room_type === 'clone')
 .sort((a, b) => a.room_code.localeCompare(b.room_code));
 const mom = opsRooms
 .filter(r => r.room_type === 'mother')
 .sort((a, b) => a.room_code.localeCompare(b.room_code));

 // Featured = closest to harvest (lowest positive days, or most overdue)
 const featured = [...flower]
 .filter(r => r.occupancy_status !== 'empty')
 .sort((a, b) => {
 const aDays = a.section_days_to_harvest ?? a.days_to_harvest ?? Infinity;
 const bDays = b.section_days_to_harvest ?? b.days_to_harvest ?? Infinity;
 return aDays - bDays;
 })[0] ?? flower[0];
 ...
 vegRooms: veg,
 momRooms: mom,
 defaultFeaturedId: featured?.room_id ?? null,
 };
 }, [opsRooms]);

 // Active featured room: user-selected or auto-computed
 ...
 // Two-stage click from grid tiles: first click = feature, second = expand
 const handleTileClick = useCallback((roomId: string, code: string) => {
 if (activeFeaturedId === roomId) {
 // Second click on featured room → expand to full view
 setSelectedRoomCode(code);
 } else {
 // First click → make this the featured room
 setFeaturedRoomId(roomId);
 }
 }, [activeFeaturedId]);

 // Direct expand (from AttentionStrip, LaborOverview — skip featuring)
 ...
 <div>
 <h1 className="font-mono uppercase tracking-[0.18em] text-sm text-cult-text-primary">Command Center</h1>
 ...
 {/* Veg + Mom rooms row */}
 {(vegRooms.length > 0 || momRooms.length > 0) && (
 <div className="border-t border-cult-border-faint pt-4">
 <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
 {vegRooms.map(room => (
 <RoomTile
 key={room.room_id}
