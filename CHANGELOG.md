...
**Type:** DOCUMENTATION
**Module:** Cultivation (new module — not yet implemented)
**Priority:** High — enables Sessions C-2 (migrations) and C-3 (UI) to proceed
**Impact:** No code or migration changes; documentation only
**Status:** COMPLETE
**Files Changed:** 8 files (3 new, 5 updated)

### Summary

Produced the complete specification for the Cultivation module. All architectural decisions are locked. Sessions C-2 and C-3 can now proceed without re-deriving design choices.

**New files:**
- `docs/CULTIVATION.md` — scope, entity map, lifecycle, grow rooms, plant groups, harvest sessions, harvest→batch handoff, compliance fields, UI screens, navigation integration
- `docs/CULTIVATION-ARCHITECTURE.md` — full DB schema (4 tables), RLS policies, 6 triggers with complete SQL, 3-migration deployment plan, frontend module structure, service layer signatures, TypeScript type definitions
- `docs/CULTIVATION-RULES.md` — 10 locked invariants, 6 architectural decisions with rationale, error message table, test requirements checklist

**Key decisions locked:**
- Group-level plant tracking (not individual RFID tags)
- Batch number is auto-generated by harvest completion trigger (resolves GAP-017)
- Same-strain same-day harvests share a batch number (ON CONFLICT DO NOTHING)
- Grow rooms managed in Settings, not the Cultivation main screen
- Harvest session is two-step: data entry → confirm completion
- Manual batch creation via BatchManagement.tsx remains supported for edge cases

**Updated files:** `AI-SESSION-BRIEF.md`, `AI-BUILD-SESSION-CHECKLIST.md`, `SYSTEM-WORKFLOW.md` (v2.7), `BATCHES.md` (v2.4), `CLAUDE.md`
